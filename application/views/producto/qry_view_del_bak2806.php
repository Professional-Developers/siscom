...
<script type="text/javascript" src="<?php echo URL_JS; ?>producto/jsProductoQryDel.js" charset=UTF-8"></script>
<style>
    td.opacity-image {
        opacity: 0.7;
...
<div id="qry_lista_producto_del">
    ...
    if ($informacion) {
        ?>
        <table id="qry_producto_del" cellpadding="0" cellspacing="0" border="0" class="dynamicTable display table table-bordered" width="100%">
            <thead>
                <tr>
                    <th></th>
                    <th>Codigo</th>
...
<!--                        <td>
                        ...
                        <td>
                            <!--<a title="Caracteristicas" class="optCaracteristicas iconic-icon-cog " onclick="panelAddCaracteristica('producto/panel_addCaracteristica', '<php echo htmlspecialchars(json_encode(array("nProId" => $data['nProId']))); ?>', '')" style="cursor: pointer;" >-->
...
<!--                        <td>
                        ...
                        <td>
                            <a title="editar" class="optEditarProducto iconic-icon-pen-alt2" onclick="panelUpdProductoDel('producto/panel_updProducto', '<?php echo htmlspecialchars(json_encode(array("nProdHueco" => $data['codigogenerado']))); ?>', '')" style="cursor: pointer;" >
                            </a>
                        </td>
                        <td>
                            <?php //  if ($data["cProHueEstado"] == "A") { ?>
<!--                                <a class="iconic-icon-check-alt" title='Activo' onClick="panelActiveProducto(<?php echo $data['codigogenerado']; ?>)" style="cursor: pointer;">
                                </a>-->
                            <?php // } else { ?>
                                <a class="iconic-icon-x" title='Inactivo' onClick="panelActiveProducto(<?php echo $data['codigogenerado']; ?>)" style="cursor: pointer;">
                                </a>
                            <?php // } ?>
                        </td>
                    </tr>

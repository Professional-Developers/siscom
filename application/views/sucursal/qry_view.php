<script type="text/javascript" src="<?php echo URL_JS; ?>sucursal/jsSucursalQry.js" charset=UTF-8"></script>
<div id="qry_lista_empressa">
    <input type="hidden" id="codSucursal" name="codSucursal" />
    <?php if ($informacion) {
        $contador=1;
        ?>
        <table id="qry_sucursal" cellpadding="0" cellspacing="0" border="0" class="dynamicTable display table table-bordered" width="100%">
            <thead>
                <tr>
                    <th></th>
                    <th>Sucursal</th>
                    ...
                    <tr class='todoMayuscula'>
                        ...
                        <td>
                            <?php if ($data["estadoSucursal"] == "1") { ?>
                                <a class="iconic-icon-check-alt" title='Activo' onClick="panelDelSucursal(<?php echo htmlspecialchars($data['nSurId']);  ?>)" style="cursor: pointer;">
                                </a>
                            <?php } else { ?>
                                <a class="iconic-icon-x" title='Inactivo' onClick="panelDelSucursal(<?php echo htmlspecialchars($data['nSurId']);  ?>)" style="cursor: pointer;">
                                </a>
                            <?php } ?>
                        </td>
                    </tr>
